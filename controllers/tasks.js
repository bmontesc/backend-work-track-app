...
tasksRouter.get('/year/:year/quarter/:quarter', (req, res)=>{
    ...
    connection.query(
        `SELECT T.id, T.plan_id, P.date, C.code, C.name, T.type, T.status,
        A.reference, T.estimated_time, T.used_time, timediff(T.estimated_time, T.used_time) AS time_diff, T.finish_date
  ...
  tasksRouter.get('/accountants/:acc_id', (req, res)=>{
    ...
    connection.query(
      `SELECT T.id, P.date, C.code, C.name, T.type, T.status,
      A.reference, T.estimated_time, T.used_time, T.finish_date
      FROM (SELECT * FROM Tasks WHERE accountant_id=? ) AS T
      LEFT JOIN (SELECT * FROM Plans) AS P ON T.plan_id=P.id
      LEFT JOIN (SELECT * FROM Accountants) AS A ON T.accountant_id=A.id
      LEFT JOIN (SELECT * FROM Companies) AS C ON P.company_id=C.id`,
      [acc_id],
      function (err, rows, fields) {
  ...
  tasksRouter.get('/companies/:com_id', (req, res)=>{
    ...
    connection.query(
      `SELECT T.id, P.date, C.code, C.name, T.type, T.status,
      A.reference, T.estimated_time, T.used_time, T.finish_date
      FROM (SELECT * FROM Plans WHERE company_id=? ) AS P
      LEFT JOIN (SELECT * FROM Tasks) AS T ON T.plan_id=P.id
      LEFT JOIN (SELECT * FROM Accountants) AS A ON T.accountant_id=A.id
      LEFT JOIN (SELECT * FROM Companies) AS C ON P.company_id=C.id`,
      [com_id],
